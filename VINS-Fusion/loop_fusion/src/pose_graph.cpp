...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (SAVE_LOOP_PATH)
    {
        ofstream loop_path_file("./vins_result/loop_result.csv", ios::app);
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            printf("optimize pose graph 4Dof\n");
            TicToc tmp_t;
            m_keyframelist.lock();
...
void PoseGraph::optimize6DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            printf("optimize pose graph 6Dof\n");
            TicToc tmp_t;
            m_keyframelist.lock();
            KeyFrame* cur_kf = getKeyFrame(cur_index);
...
void PoseGraph::updatePath()
{
    ...
    if (SAVE_LOOP_PATH)
    {
        ofstream loop_path_file_tmp("./vins_result/loop_result.csv", ios::out);
