            ...
            if(bDetected)
            {
                ...
                if(mbLoopDetected)
                {
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        if (fabs(phi(0))<0.04f && fabs(phi(1))<0.04f && fabs(phi(2))<0.349f)//Awen: original numbers are 0.008f, 0.008f, 0.349f
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                                // If inertial, force only yaw
                        ...
                        {
                            cout << "BAD LOOP!!! \nfabs(phi(0))=" << fabs(phi(0)) << "\nfabs(phi(1))="<< fabs(phi(1)) << "\nfabs(phi(2))="<<fabs(phi(2))<< endl;
                        }
                    }
                    else
                    {
