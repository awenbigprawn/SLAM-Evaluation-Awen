...
DEFINE_bool(log_output, true, "Log output to CSV files.");
...
Pipeline::Pipeline(const VioParams& params,
  ...
  std::cout<<"Now Pipeline is established and lauchThread() is beginning!"<<std::endl;//Awen: now lauchThread()
  launchThreads();
}
...
void Pipeline::spinOnce(StereoImuSyncPacket::UniquePtr stereo_imu_sync_packet) {
  ...
  if (!shutdown_) {
    ...
    //std::cout<< "Awen: pipeline spinOnce()" <<std::endl;
    //std::cout<< "pipeline:spinonce()----------------------------------------------------------" <<std::endl;
    stereo_frontend_input_queue_.pushBlockingIfFull(
        std::move(stereo_imu_sync_packet), 5u);

    if (!parallel_run_) {
      // Run the pipeline sequentially.
      std::cout<<"Awen: Run the pipeline sequentially. ---spinSequential()"<<std::endl;
      spinSequential();
    }
  } else {
...
void Pipeline::spinSequential() {
  ...
  std::cout<<"Awen: vio_frontend_module_->spin();"<<std::endl;//Awen
  vio_frontend_module_->spin();

  CHECK(vio_backend_module_);
  std::cout<<"Awen: vio_backend_module_->spin();"<<std::endl;//Awen
  vio_backend_module_->spin();

  if (mesher_module_) {mesher_module_->spin();
  std::cout<<"Awen: mesher_module_->spin();"<<std::endl;}

  if (lcd_module_) {lcd_module_->spin();
  std::cout<<"Awen: lcd_module_->spin();"<<std::endl;}

  if (visualizer_module_) {visualizer_module_->spin();
  std::cout<<"Awen: visualizer_module_->spin();"<<std::endl;}

  if (display_module_) {display_module_->spin();
  std::cout<<"Awen: display_module_->spin();"<<std::endl;}
}
