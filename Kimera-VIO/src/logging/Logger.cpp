...
void BackendLogger::logBackendTiming(const BackendOutput& output) {
  ...
  if (!is_header_written) {
    output_stream << "#cur_kf_id,factorsAndSlotsTime,preUpdateTime,"
                  ...
                  << "linearizeMarginalizeTime,marginalizeTime,backendOptTime,"
                  << "Create_Backend_Output_Payload_time,getMapLmkIdsTo3dPointsInTimeHorizon_time,"
                  <<"MANAGE_IMU_MEASUREMENTS_time_,MANAGE_VISION_MEASUREMENTS_time" << std::endl;
    is_header_written = true;
  }
  ...
  output_stream << output.cur_kf_id_ << ","
                ...
                << output.debug_info_.backendOptTime_<<","
                //<< output.debug_info_.Create_Backend_Output_Payload_time_<<","//Awen
                //<< output.debug_info_.getMapLmkIdsTo3dPointsInTimeHorizon_time_//Awen
                << output.debug_info_.MANAGE_IMU_MEASUREMENTS_time_ <<","//Awen
                << output.debug_info_.MANAGE_VISION_MEASUREMENTS_time_//Awen
                << std::endl;
}
...
void FrontendLogger::logFrontendStats(
    ...
    const double& full_preint_duration,//AwenProblem!
  ...
  if (!is_header_written) {
    output_stream_stats << "#timestamp_lkf,mono_status,stereo_status,"
                        ...
                        //if keyframe:
                          ...
                          << "Feature_Detection_First_Part_,"
                          << "Feature_Detection_Second_Part_,"
                          << "Feature_Detection_Third_Part_,"
                          << "Feature_Detection_Fourth_Part_,"
                          << "Feature_Detection_Fifth_Part_,"
                        << std::endl;
    is_header_written = true;
  }
  ...
  output_stream_stats
      ...
      << ","
      // Info about timing.
      << featureDetector_info.featureDetectionTime_ << ","//AwenProblem!
      ...
      << featureDetector_info.extracted_corners_ << "," << featureDetector_info.need_n_corners_ << ","
      << frontEndTotalSpinTime << "," << full_preint_duration << "," 
      << Tracking_processStereoFrame_time << ","
      << clone_rect_params_time_<< ","
            << feature_tracking_time_ << ","
            << some_between_definition_time_ << ","
            //if keyframe:
              ...
              << sparseStereoMatching_time_ <<","
              //<< get_smart_stereo_meas_time_
              << featureDetector_info.Feature_Detection_First_Part_ <<","
              << featureDetector_info.Feature_Detection_Second_Part_ << ","
              << featureDetector_info.Feature_Detection_Third_Part_ <<","
              << featureDetector_info.Feature_Detection_Fourth_Part_ << ","
              << featureDetector_info.Feature_Detection_Fifth_Part_
      << std::endl;//AwenProblem!
}
...
void PipelineLogger::logPipelineOverallTiming(
  ...
  outputFile_timingOverall_ << "vio_overall_time [micro s]" << std::endl;
...
void LoopClosureDetectorLogger::logDebugInfo(const LcdDebugInfo& debug_info) {
  ...
  if (!is_header_written) {
    output_stream_status << "#timestamp_kf,lcd_status,query_id,match_id,"
                         ...
                         << "pgo_size,pgo_lc_count,pgo_lc_inliers,update_duration_" << std::endl;
    is_header_written = true;
  }
  ...
  output_stream_status << debug_info.timestamp_ << ","
                       ...
                       << debug_info.update_duration_<< std::endl;
}
