...
BackendOutput::UniquePtr VioBackEnd::spinOnce(const BackendInput& input) {
  ...
  if (backend_status) {
    ...
    //auto a_start_time = utils::Timer::tic();//-----------------------------------------------------------------------------------\tic
    ...
    //debug_info_.getMapLmkIdsTo3dPointsInTimeHorizon_time_ = utils::Timer::toc(a_start_time).count();//----------------------------------/toc
    //std::cout << "Awen: backend getMapLmkIdsTo3dPointsInTimeHorizon_time = "<< getMapLmkIdsTo3dPointsInTimeHorizon_time << std::endl;
    //a_start_time = utils::Timer::tic();//-----------------------------------------------------------------------------------\tic
    ...
    //debug_info_.Create_Backend_Output_Payload_time_ = utils::Timer::toc(a_start_time).count();//-----------------------------------------------------------/toc
    //std::cout << "Awen: Create_Backend_Output_Payload_time_ = "<< debug_info_.Create_Backend_Output_Payload_time_ << std::endl;
...
bool VioBackEnd::addVisualInertialStateAndOptimize(
  ...
  ++curr_kf_id_;
  std::cout << "addVisualInertialStateAndOptimize is used once!---------------------"<<std::endl;
  VLOG(1) << "VIO: adding keyframe " << curr_kf_id_
          << " at timestamp:" << UtilsNumerical::NsecToSec(timestamp_kf_nsec)
          << " (nsec).";
  auto some_start_time = utils::Timer::tic();//------------------------------------------------------------------------\tic
  ...
  debug_info_.MANAGE_IMU_MEASUREMENTS_time_ = utils::Timer::toc(some_start_time).count();//----------------------------------/toc
  std::cout << "Awen: backend MANAGE_IMU_MEASUREMENTS_time = "<< debug_info_.MANAGE_IMU_MEASUREMENTS_time_ << std::endl;
  some_start_time = utils::Timer::tic();//-----------------------------------------------------------------------------\tic
  ...
  double MANAGE_VISION_MEASUREMENTS_time = utils::Timer::toc(some_start_time).count();//----------------------------------/toc
  std::cout << "Awen: backend MANAGE_VISION_MEASUREMENTS_time = "<< MANAGE_VISION_MEASUREMENTS_time << std::endl;
...
bool VioBackEnd::optimize(
  ...
  const auto& total_start_time = utils::Timer::tic();//--------------------------------------------------\tic
  ...
  if (VLOG_IS_ON(10) || log_output_) {
    ...
    //std::cout<<"Awen: Now factorsAndSlotsTime is recorded once ---------------------debug_info_.factorsAndSlotsTime_: " 
     //         << debug_info_.factorsAndSlotsTime_ << "(microseconds)\n";
  ...
  if (VLOG_IS_ON(10) || log_output_) {
    debug_info_.updateTime_ =
        //utils::Timer::toc<std::chrono::seconds>(start_time).count();
        utils::Timer::toc(start_time).count();
        //std::cout<<"Awen: Now updateTime is recorded once -----------debug_info_.updateTime_: "<< debug_info_.updateTime_ << "(microsec)\n";
    start_time = utils::Timer::tic();
  }
  ...
  if (is_smoother_ok) {
    ...
    if (VLOG_IS_ON(5) || log_output_) {
      debug_info_.updateSlotTime_ =
          //utils::Timer::toc<std::chrono::seconds>(start_time).count();
          utils::Timer::toc(start_time).count();//--------------------------------------------------------------/toc
      start_time = utils::Timer::tic();
    }
  ...
  debug_info_.backendOptTime_ = utils::Timer::toc(total_start_time).count();//---------------------------------/toc
  return is_smoother_ok;
}
...
void VioBackEnd::postDebug(
  ...
  if (VLOG_IS_ON(10)) {
    ...
    const auto& end_time =
        ...
        utils::Timer::toc(total_start_time).count();
