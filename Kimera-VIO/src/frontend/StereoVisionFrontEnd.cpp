...
StereoVisionFrontEnd::StereoVisionFrontEnd(
  ...
  //frontendState_ = 0;//Awen
  // Instantiate FeatureDetector
  // AwenProblem!
  //std::cout << "Awen: Instantiate FeatureDetector!" << std::endl;
  feature_detector_ = VIO::make_unique<FeatureDetector>(
      frontend_params.feature_detector_params_/*, tracker_.debug_info_*/);
  ...
  std::cout << "Awen: now a StereoVisionFrondEnd is builded!!!!"<<std::endl;
}

/* -------------------------------------------------------------------------- */
FrontendOutput::UniquePtr StereoVisionFrontEnd::spinOnce(
    const StereoFrontEndInputPayload& input) {
  //std::cout<< "Awen: StereoVisionFrontEnd spinOnce()" <<std::endl;//Awen
  //auto tic_total_spin_time = utils::Timer::tic();//Awen
  switch (frontend_state_) {
    case FrontendState::Bootstrap: {
      //if (utils::Timer::doItOnce(frontendState_))//Awen
      //std::cout<<"Awen: frondend_state_ = Bootstrap" <<std::endl;
      return bootstrapSpin(input);
    } break;
    case FrontendState::Nominal: {
      //if (utils::Timer::doItOnce(frontendState_))//Awen
      //std::cout<<"Awen: frondend_state_ = Nominal" <<std::endl;
...
FrontendOutput::UniquePtr StereoVisionFrontEnd::nominalSpin(
  ...
  auto total_start_time = utils::Timer::tic();
  ...
  // into account!!!).
  auto tic_full_preint = utils::Timer::tic();//----------------------------------------------\\\\\\\\\\\\\\\\\\\\\tic
  ...
  auto full_preint_duration =
      utils::Timer::toc<std::chrono::microseconds>(tic_full_preint).count();//----------------////////////////////toc
  ...
  //tic_full_preint = utils::Timer::tic();//-----------------------------------------------------------\\\\\\\\\\\\tic
  ...
  //////////////////////////////////////////////////////////////////////////////
  //auto left_camera_rotation_rectified_time =
  //   utils::Timer::toc<std::chrono::microseconds>(tic_full_preint).count();//-----------------------//////////////toc
  //std::cout<< "left_camera_rotation_rectified_time = " << left_camera_rotation_rectified_time <<std::endl;
  ...
  auto Tracking_processStereoFrame_time =
      utils::Timer::toc<std::chrono::microseconds>(tic_full_preint).count();//-----------------------//////////////toc
  //////////////////////////////////////////////////////////////////////////////
  ...
  if (stereoFrame_km1_->isKeyframe()) {
    ...
    totalSpinTime_ = utils::Timer::toc<std::chrono::microseconds>(total_start_time).count();//Awen
    ////////////////// DEBUG INFO FOR FRONT-END ////////////////////////////////
    if (logger_) {
      logger_->logFrontendStats(
          stereoFrame_lkf_->getTimestamp(),
          getTrackerInfo(),
          getFeatureDetectorInfo(),//AwenProblem!
          totalSpinTime_,//Awen
          full_preint_duration,//Awen
          Tracking_processStereoFrame_time,//Awen
            clone_rect_params_time_,
            feature_tracking_time_,
            some_between_definition_time_,
            //if keyframe:
              sparse_stereo_time_,
              outlierRejectionStereo_time_,
              feature_detection_in_Traking_processStereoFrame_time_,
              sparseStereoMatching_time_,
              //get_smart_stereo_meas_time_,
          trackerStatusSummary_,          
    ...
    //std::cout <<"timing_stats_keyframe_rate: "<<utils::Timer::toc(start_time).count()<<std::endl;//Awen
    ...
    //std::cout <<"timing_stats_frame_rate: "<<utils::Timer::toc(start_time).count()<<std::endl;//Awen
...
StatusStereoMeasurementsPtr StereoVisionFrontEnd::processStereoFrame(
  ...
  auto start_time = utils::Timer::tic();//------------------------------------------------------------------------------\tic1
  stereoFrame_k_->cloneRectificationParameters(*stereoFrame_km1_);
  clone_rect_params_time_ = utils::Timer::toc(start_time).count();//Awen: before: double clone_rect_params_time---------/toc1

  start_time = utils::Timer::tic();//-----------------------------------------------------------------------------------\tic2
  /////////////////////// TRACKING /////////////////////////////////////////////
  ...
  //////////////////////////////////////////////////////////////////////////////
  feature_tracking_time_ = utils::Timer::toc(start_time).count();//Awen--------------------------------------------/toc2
  start_time = utils::Timer::tic();//------------------------------------------------------------------------------------\tic3
  ...
  some_between_definition_time_ = utils::Timer::toc(start_time).count();//Awen-------------------------------------/toc3
  ...
  if (max_time_elapsed || nr_features_low || stereoFrame_k_->isKeyframe()) {
    ...
    sparse_stereo_time_ = 0;
    outlierRejectionStereo_time_ = 0;
    if (tracker_.tracker_params_.useRANSAC_) {
      ...
      // get 3D points via stereo
      start_time = utils::Timer::tic();//-----------------------------------------------------------------------------------\tic4
      stereoFrame_k_->sparseStereoMatching();
      sparse_stereo_time_ = utils::Timer::toc(start_time).count();
      sparse_stereo_time = utils::Timer::toc(start_time).count();//---------------------------------------------------------/toc4
      start_time = utils::Timer::tic();//-----------------------------------------------------------------------------------\tic5
      ...
      outlierRejectionStereo_time_ = utils::Timer::toc(start_time).count();//------------------------------------------------/toc5
    } else {
    ...
    start_time = utils::Timer::tic();//--------------------------------------------------------------------------------------\tic6
    CHECK(feature_detector_);
    feature_detector_->featureDetection(left_frame_k);
    feature_detection_in_Traking_processStereoFrame_time_ = utils::Timer::toc(start_time).count();//-------------------------/toc6

    // Get 3D points via stereo, including newly extracted
    // (this might be only for the visualization).
    start_time = utils::Timer::tic();//--------------------------------------------------------------------------------------\tic7
    stereoFrame_k_->sparseStereoMatching();
    sparse_stereo_time += utils::Timer::toc(start_time).count();
    sparseStereoMatching_time_ = utils::Timer::toc(start_time).count();//----------------------------------------------/toc7
    ...
    start_time = utils::Timer::tic();//------------------------------------------------------------------------------\tic8
    smart_stereo_measurements = getSmartStereoMeasurements(*stereoFrame_k_);
    get_smart_stereo_meas_time_ = utils::Timer::toc(start_time).count();//-------------------------------------/toc8

    VLOG(2) << "timeClone: " << clone_rect_params_time_ << '\n'//Awen
            << "timeSparseStereo: " << sparse_stereo_time << '\n'
            << "timeGetMeasurements: " << get_smart_stereo_meas_time_;
  } else {
    stereoFrame_k_->setIsKeyframe(false);
  }

  // Update keyframe to reference frame for next iteration.
  start_time = utils::Timer::tic();//----------------------------------------------------------tic reset
  ...
  std::cout << "Awen: reset frame time = "<< utils::Timer::toc(start_time).count() << std::endl;//--------------------toc reset
}
